...
impl BlockMetadataFetcher {
    pub fn new(rpc_url: String) -> Self {
        ...
        let client = RpcClient::builder().layer(retry_layer).layer(delay_layer).http(Url::parse(rpc_url.as_str()).unwrap());
    ...
    pub async fn stream_blocks(&self) -> eyre::Result<impl Stream<Item = eyre::Result<BlockMetadata>>> {
        let latest_block = self.provider.get_block_by_number(BlockNumberOrTag::Latest).await?;
        let latest_block_number = latest_block.ok_or(eyre::eyre!("Failed to get latest block"))?.header.number();       
        let block_stream = self.create_block_metadata_stream(latest_block_number).await;
        Ok(block_stream)
    }

    pub async fn create_block_metadata_stream(&self, start_block_number: u64) -> impl Stream<Item = eyre::Result<BlockMetadata>> {
        try_stream! {
...
pub async fn stream_blocks_to_clickhouse(rpc_url: String) -> eyre::Result<()> {
  ...
  clickhouse_service.write_block_metadata_stream(block_stream).await?;
  Ok(())
}