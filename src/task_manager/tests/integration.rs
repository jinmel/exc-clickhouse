...
async fn test_end_to_end_task_lifecycle() {
    ...
    let success_task = manager.spawn_task("success_task".to_string(), || {
        Box::pin(async {
    ...
    let failure_task = manager.spawn_task("failure_task".to_string(), || {
        Box::pin(async {
    ...
    let slow_task = manager.spawn_task("slow_task".to_string(), || {
        Box::pin(async {
...
async fn test_resource_limit_boundary_testing() {
    ...
    for i in 0..10 {
        let task_id = manager.spawn_task(format!("limited_task_{}", i), || {
            Box::pin(async {
                sleep(Duration::from_millis(100)).await;
                Ok(())
            })
