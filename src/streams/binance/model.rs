...
impl TryInto<NormalizedQuote> for BookTickerEvent {
    ...
    fn try_into(self) -> Result<NormalizedQuote, Self::Error> {
        ...
        let ask_amount = self.best_ask_quantity.parse::<f64>().map_err(|e| {
            ExchangeStreamError::MessageError(format!("Invalid ask quantity value: {e}"))
        })?;

        let ask_price = self.best_ask.parse::<f64>().map_err(|e| {
            ExchangeStreamError::MessageError(format!("Invalid ask price value: {e}"))
        })?;

        let bid_price = self.best_bid.parse::<f64>().map_err(|e| {
            ExchangeStreamError::MessageError(format!("Invalid bid price value: {e}"))
        })?;

        let bid_amount = self.best_bid_quantity.parse::<f64>().map_err(|e| {
            ExchangeStreamError::MessageError(format!("Invalid bid quantity value: {e}"))
        })?;
